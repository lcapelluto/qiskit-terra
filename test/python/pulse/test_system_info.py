...
class TestSystemInfo(QiskitTestCase):
    ...
    #     sysinfo = SystemInfo(self.backend, default_ops={('tmp', 0): sched})
    #     self.assertTrue(sysinfo._defaults.has('tmp', 0))
    ...
    def test_coupling_map(self):
        ...
        self.assertEqual(self.backend.configuration().coupling_map, {0: {1}})
    ...
    def test_freq_est(self):
        ...
        self.assertEqual(self.backend.defaults().qubit_freq_est(1),
                         5.0 * 1e9)
        self.assertEqual(self.backend.defaults().meas_freq_est(0),
                         6.5 * 1e9)
    ...
    def test_ops(self):
        """Test `ops`."""
        self.assertEqual(self.backend.defaults().ops, ['u1', 'u3', 'cx', 'measure'])

    def test_op_qubits(self):
        """Test `op_qubits`."""
        self.assertEqual(self.backend.defaults().op_qubits('u1'), [0])
        self.assertEqual(self.backend.defaults().op_qubits('u3'), [0, 1])
        self.assertEqual(self.backend.defaults().op_qubits('cx'), [(0, 1)])
        self.assertEqual(self.backend.defaults().op_qubits('measure'), [(0, 1)])

    def test_qubit_ops(self):
        """Test `qubit_ops`."""
        self.assertEqual(self.backend.defaults().qubit_ops(0), ['u1', 'u3'])
        self.assertEqual(self.backend.defaults().qubit_ops(1), ['u3'])
        self.assertEqual(self.backend.defaults().qubit_ops((0, 1)), ['cx', 'measure'])
    ...
    def test_get_op(self):
        ...
        self.assertIn('tmp', self.backend.defaults().ops)
        self.assertEqual(self.backend.defaults().op_qubits('tmp'), [0, 1])
    ...
    def test_remove(self):
        ...
        with self.assertRaises(PulseError):
            self.backend.defaults().remove('not_there', (0,))
    ...
    def test_parameterized_schedule(self):
        ...
        self.backend.defaults().add('pv_test', 0, converted_instruction)
        self.assertEqual(self.backend.defaults().get_parameters('pv_test', 0), ('P1', 'P2'))

        sched = self.backend.defaults().get('pv_test', 0, P1=0, P2=-1)
        self.assertEqual(sched.instructions[0][-1].command.value, -1)
        with self.assertRaises(PulseError):
            self.backend.defaults().get('pv_test', 0, 0, P1=-1)
        with self.assertRaises(PulseError):
            self.backend.defaults().get('pv_test', 0, P1=1, P2=2, P3=3)
    ...
    def test_sequenced_parameterized_schedule(self):
        ...
        self.backend.defaults().add('inst_seq', 0, ParameterizedSchedule(*converted_instruction,
                                                            name='inst_seq'))

        with self.assertRaises(PulseError):
            self.backend.defaults().get('inst_seq', 0, P1=1, P2=2, P3=3, P4=4, P5=5)

        with self.assertRaises(PulseError):
            self.backend.defaults().get('inst_seq', 0, P1=1)

        with self.assertRaises(PulseError):
            self.backend.defaults().get('inst_seq', 0, 1, 2, 3, P1=1)

        sched = self.backend.defaults().get('inst_seq', 0, 1, 2, 3)
        ...
        sched = self.backend.defaults().get('inst_seq', 0, P1=1, P2=2, P3=3)
        self.assertEqual(sched.instructions[0][-1].command.phase, 1)
        self.assertEqual(sched.instructions[1][-1].command.phase, 2)
        self.assertEqual(sched.instructions[2][-1].command.phase, 3)

        sched = self.backend.defaults().get('inst_seq', 0, 1, 2, P3=3)
        self.assertEqual(sched.instructions[0][-1].command.phase, 1)
        self.assertEqual(sched.instructions[1][-1].command.phase, 2)
        self.assertEqual(sched.instructions[2][-1].command.phase, 3)
    ...
    def test_build_cmd_def(self):
        ...
        self.assertTrue(self.backend.defaults().has('u1', (0,)))
        self.assertTrue(self.backend.defaults().has('u3', (0,)))
        self.assertTrue(self.backend.defaults().has('u3', 1))
        self.assertTrue(self.backend.defaults().has('cx', (0, 1)))
        self.assertEqual(self.backend.defaults().get_parameters('u1', 0), ('P1',))
        u1_minus_pi = self.backend.defaults().get('u1', 0, P1=1)
