...
class TestQASMQobj(QiskitTestCase):
    ...
    def test_gate_calibrations_to_dict(self):
        ...
        pulse_library = [PulseLibraryItem(name='test', samples=[1, 1, 1, 1])]
        valid_qobj = QasmQobj(
            qobj_id='12345',
            header=QobjHeader(),
            config=QasmQobjConfig(shots=1024, memory_slots=2, max_credits=10,
                ...
                QasmQobjExperiment(
                    ...
                    config=QasmQobjConfig(
                        calibrations=QasmExperimentCalibrations(
                            gates=[GateCalibration(name='u1', qubits=[1], params=[0.4], instructions=[])]
                        )
                    )
                )
            ]
        ...
        expected_dict = {
            ...
            'header': {},
            'config': {'max_credits': 10, 'memory_slots': 2, 'shots': 1024,
                       'pulse_library': [{'name': 'test', 'samples': [1, 1, 1, 1]}]},
            'experiments': [
                {'instructions': [
                    {'name': 'u1', 'params': [0.4], 'qubits': [1]}
                ],
                 'config': {'calibrations':
                     {'gates': [{'name': 'u1', 'qubits': [1], 'params': [0.4], 'instructions': []}]}},
                 'header': {}}
            ],
        }
        self.assertEqual(expected_dict, res)
